// app/@modal/(.)projects/[project]/page.tsx
'use client';
...
export default function ProjectModal({
  ...
  return (
    <div
      className={`fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-25 transition-opacity duration-300 ${
        isVisible ? 'opacity-100' : 'opacity-0'
      }`}
      onClick={onDismiss}
    >
      <div
        className={`mx-auto mt-20 w-11/12 overflow-hidden rounded-md bg-background shadow-xl transition-all duration-500 ${
          isVisible ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* <h2 className="mb-4 text-2xl font-bold">Project: {project}</h2>
        <p>This is the content for project {project}.</p> */}
                  ...
                  <div>
                    ...
                    >
                      <a
                        ...
                        className="text-foreground/60 duration-300 ease-in-out hover:text-foreground"
                        target="_blank"
                      >
                        <svg
                          className="fill-current"
